def arithmetic_arranger(problems, show_result = False):
    ...
    for i in range(0, len(problems)):
        ...
        if len_diff < 0:
            len_diff = len_diff * -1
            top_line = top_line + space_filler(len_diff) + val1 
            down_line = down_line + operator + " " + val2 
            dash_line = space_filler(len(str(val2))+2, "-")
        else:
            top_line = top_line + space_filler(len_diff + 1)+ val1 + "    "
            down_line = down_line + operator + " " + space_filler(len_diff) + val2 + "    "
            dash_line = space_filler(len(str(val2))+len_diff+ 2, "-") + "    "  
...
print(arithmetic_arranger(["356 - 1369" ], False))
